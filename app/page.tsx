...
import { Moon, Heart, Home as HomeIcon, ArrowRight, Globe, Menu, X, ChevronDown, Check, Sparkles } from 'lucide-react'
...
const translations: Record<string, any> = {
  ...
  ja: {
    ...
    hero: { subtitle: '私たちは猫と犬と共に', title: '何気ない日常を、\n真剣に考える', description: '絆に焦点を当てたペットライフスタイルブランド。', cta: 'コレクションを見る' },
    story: { label: 'Brand Story' },
    products: { label: 'コレクション', sleepwear: { title: 'クラシックペットパジャマ', description: '柔らかく肌に優しい', price: '$32 - $48' }, homewear: { title: 'ホームシェアリング', description: '人とペットの共有スペース', price: '近日公開' }, notify: '発売通知を受け取る', comingSoon: '最初のコレクションは近日発売' },
    values: { warmth: { title: '温かさ', desc: 'ファストファッションでもトレンドでもない' }, companionship: { title: '絆', desc: '静かな夜すべてが、真剣に扱われる価値がある' }, daily: { title: '日常', desc: '抑制された美しさ' } },
    footer: { tagline: '絆に焦点を当てたペットライフスタイルブランド', nav: 'ナビゲーション', contact: 'お問い合わせ', contactText: 'ご質問は、こちらまで', copyright: '© 2026 Nightfall. All rights reserved.', motto: '彼らと過ごす每一天を無駄にしない。' }
  },
  ko: {
    nav: { story: '스토리', products: '컬렉션', about: '소개' },
    hero: { subtitle: '우리는 고양이와 강아지와', title: '일상적인 동반자,\n진지하게', description: '동반자에 초점을 맞춘 펫 라이프스타일 브랜드.', cta: '컬렉션 보기' },
    story: { label: 'Brand Story' },
    products: { label: '컬렉션', sleepwear: { title: '클래식 펫 잠옷', description: '부드럽고 피부 친화적', price: '$32 - $48' }, homewear: { title: '홈 쉐어링', description: '사람과 반려동물의 공유 공간', price: '곧 출시' }, notify: '출시 알림 받기', comingSoon: '첫 번째 컬렉션 곧 출시' },
    values: { warmth: { title: '따뜻함', desc: '빠른 패션도, 트렌드도 없습니다' }, companionship: { title: '동반자', desc: '조용한 밤 모두 진지하게' }, daily: { title: '일상', desc: '절제된 미학' } },
    footer: { tagline: '동반자에 초점을 맞춘 펫 라이프스타일 브랜드', nav: '탐색', contact: '연락처', contactText: '질문이나 제안', copyright: '© 2026 Nightfall. All rights reserved.', motto: '그들과 함께하는 매일을 낭비하지 않습니다.' }
  },
  fr: {
    nav: { story: 'Histoire', products: 'Collection', about: 'À propos' },
    hero: { subtitle: 'Nous vivons avec chats et chiens', title: 'La Compagnie Quotidienne,\nPrise au Sérieux', description: 'Une marque de lifestyle pour animaux.', cta: 'Explorer la Collection' },
    story: { label: 'Brand Story' },
    products: { label: 'Collection', sleepwear: { title: 'Pyjama pour Animaux', description: 'Doux et respectueux de la peau', price: '$32 - $48' }, homewear: { title: 'Partage Domestique', description: 'Espace partagé', price: 'Bientôt' }, notify: 'Être Averti', comingSoon: 'Première collection bientôt' },
    values: { warmth: { title: 'Chaleur', desc: 'Pas de fast fashion. Designs chaleureux.' }, companionship: { title: 'Compagnie', desc: 'Chaque nuit calme mérite sérieux.' }, daily: { title: 'Quotidien', desc: 'Esthétique sobre.' } },
    footer: { tagline: 'Marque de lifestyle pour animaux', nav: 'Navigation', contact: 'Contact', contactText: 'Questions ?', copyright: '© 2026 Nightfall. Tous droits réservés.', motto: 'Ne pas gaspiller un jour avec eux.' }
  },
  de: {
    nav: { story: 'Geschichte', products: 'Kollektion', about: 'Über Uns' },
    hero: { subtitle: 'Wir leben mit Katzen und Hunden', title: 'Alltägliche Begleitung,\nErnst Genommen', description: 'Eine Haustier-Lifestyle-Marke.', cta: 'Kollektion Entdecken' },
    story: { label: 'Brand Story' },
    products: { label: 'Kollektion', sleepwear: { title: 'Haustier-Nachtwäsche', description: 'Weich und hautfreundlich', price: '$32 - $48' }, homewear: { title: 'Wohnbereich', description: 'Gemeinsamer Raum', price: 'Demnächst' }, notify: 'Benachrichtigen', comingSoon: 'Erste Kollektion demnächst' },
    values: { warmth: { title: 'Wärme', desc: 'Keine Fast Fashion. Warme Designs.' }, companionship: { title: 'Begleitung', desc: 'Jede ruhige Nacht verdient Ernst.' }, daily: { title: 'Alltag', desc: 'Zurückhaltende Ästhetik.' } },
    footer: { tagline: 'Haustier-Lifestyle-Marke', nav: 'Navigation', contact: 'Kontakt', contactText: 'Fragen?', copyright: '© 2026 Nightfall. Alle Rechte vorbehalten.', motto: 'Keinen Tag verschwenden.' }
  }
}
  ...
  return (
    <main className="min-h-screen bg-[#fafaf9] relative">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -left-40 w-96 h-96 bg-stone-200/40 rounded-full blur-3xl" />
        <div className="absolute top-1/2 -right-40 w-[500px] h-[500px] bg-gradient-to-bl from-stone-100/60 to-transparent rounded-full blur-3xl" />
        <div className="absolute -bottom-40 left-1/3 w-[600px] h-[600px] bg-stone-100/50 rounded-full blur-3xl" />
        <div className="absolute inset-0 opacity-[0.02]" style={{backgroundImage: 'radial-gradient(circle at 1px 1px, #1c1917 1px, transparent 0)', backgroundSize: '40px 40px'}} />
      </div>

      <div className="relative z-10">
        {/* Language Tip */}
        {showLangTip && (
          <div className="fixed top-0 left-0 right-0 z-[60] bg-stone-900/95 backdrop-blur text-stone-50 px-6 py-3 text-sm shadow-lg">
            <div className="max-w-6xl mx-auto flex justify-between items-center">
              <p className="opacity-90 flex items-center gap-2">
                <Sparkles size={14} className="text-stone-300" />
                <span>🌐 {lang === 'en' ? 'EN, 中文，日本語，한국어, Français, Deutsch' : lang === 'zh' ? '英语、中文、日语、韩语、法语、德语' : '6 languages available'}</span>
              </p>
              <button onClick={() => setShowLangTip(false)} className="text-stone-400 hover:text-stone-50"><X size={16} /></button>
            </div>
          </div>
        )}

        {/* Navigation */}
        <nav className={`fixed w-full bg-[#fafaf9]/85 backdrop-blur-md z-50 border-b border-stone-200/50 transition-all ${showLangTip ? 'top-10' : 'top-0'}`}>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden bg-[#fafaf9] border-t border-stone-200/50 py-4">
        ...
        <section className="relative pt-40 pb-24 px-6">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <Link href="#products" className="inline-flex items-center gap-3 bg-stone-900 text-stone-50 px-10 py-4 rounded-full hover:bg-stone-800 transition-all duration-300 group shadow-lg shadow-stone-900/20">
        ...
        <section id="story" className="py-24 px-6">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-6 text-stone-700 leading-relaxed bg-white/60 backdrop-blur-sm p-8 rounded-2xl border border-stone-100 shadow-sm">
        ...
        {/* Products */}
        <section id="products" className="py-24 px-6 bg-white/50 backdrop-blur-sm border-y border-stone-100">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div className="group">
                <div className="aspect-[4/5] bg-gradient-to-br from-stone-100 to-stone-50 rounded-sm mb-6 flex items-center justify-center relative overflow-hidden">
                  <div className="absolute inset-0 bg-stone-200/0 group-hover:bg-stone-200/50 transition-colors duration-500" />
        ...
        <section className="py-24 px-6">
          <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-12">
            <div className="text-center space-y-4">
              <div className="w-16 h-16 mx-auto rounded-full bg-gradient-to-br from-stone-200/80 to-stone-100/80 backdrop-blur flex items-center justify-center border border-stone-200/50 shadow-sm">
        ...
        <footer id="about" className="py-16 px-6 bg-stone-900 text-stone-400 relative overflow-hidden">
          <div className="absolute inset-0 opacity-5">
            <div className="absolute top-0 right-0 w-64 h-64 bg-stone-500 rounded-full blur-3xl" />
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-stone-600 rounded-full blur-3xl" />
          </div>
